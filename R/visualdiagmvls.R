...
visualdiagmvls<-function(mvls, method="multiple", cluster, norm=F){
  if(method=="multiple"){
    ...
    ggplot(data=matrix.ggplot, aes(x=time, y=V1, colour=Cluster, group=id))+geom_line(alpha=.5)+ggtitle("Pattern distribution")+labs (x="Time", y = "Values")+theme_classic()
    ...
    ggplot(data=matrix.ggplot, aes(x=time, y=V1, group=id))+geom_line(alpha=.5)+ggtitle(paste0("Pattern distribution",cluster))+labs (x="Time", y = "Values")+theme_classic()
  }
}
